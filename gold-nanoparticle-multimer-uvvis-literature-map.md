...
**Context:** 12.9 nm AuNP, **7%** polydispersity (TEM, C500 report; the earlier "4%" figure was superseded). The measured UV–Vis is broader / red-tailed than an FDTD extinction that accounts only for the size spread alone. Working hypothesis: reversible monomer ⇌ dimer ⇌ trimer ⇌ multimer equilibria in liquid, so the UV–Vis is an ensemble / time-integrated snapshot of coexisting states. Temperature-dependent UV–Vis was originally read as showing an **isosbestic point**, consistent with two (or few) interconverting optical species. ⚠️ **THIS PREMISE IS NOW IN DOUBT (2026-07-11):** the 575 nm crossing exists only under certain normalizations, and where a crossing does appear it **DRIFTS** (562 → 577 nm across the ramp) rather than staying stationary as a true isosbestic must. Its existence and location are normalization-dependent (CLAUDE.md #12/#14) — **the two-state argument cannot rest on it.** The hypothesis below is retained as the project's original framing, not as an established finding. What IS established: the spectra are red-tailed far beyond size polydispersity at ALL temperatures, and that excess grows and reverses with heating. Goal: Monte-Carlo the cluster population → compute a simulated spectrum → compare to experiment → invert into a tool that reports nominal size + size distribution (%), as an alternative to TEM.

---

## Why the hypothesis is physically sound (the short version)

A single 12 nm Au sphere has a narrow, symmetric dipolar LSPR (~520 nm) with negligible radiative broadening at this size. When two particles come within ~1 diameter, near-field coupling splits the mode: the longitudinal (bonding) mode red-shifts strongly and gains oscillator strength, while the transverse mode blue-shifts weakly. Because dimers/trimers form at **random orientations and gaps**, an ensemble of them contributes a smear of red-shifted intensity — exactly a red tail + apparent broadening on top of the monomer peak. A pure size-distribution model (FDTD with only the size spread) cannot reproduce this because it never introduces coupling. An isosbestic point *would be* the thermodynamic fingerprint of a two-state system: it appears when population shifts between two species whose spectra cross at a **fixed** wavelength. ⚠️ In THIS dataset the crossing is normalization-dependent and **drifts** with temperature, so it does not establish two-state behaviour — a drifting crossing instead suggests an *evolving* cluster geometry (cf. Cardellini 2025, ref 11: λ_iso tracks interparticle spacing).

**Three independent ML papers corroborate the effect as everyone else's "error term."** Klinavičius (ref 17) had to *sonicate agglomerates away* to get good monomer fits and explicitly warns that aggregation red-shifts and broadens the spectrum; Bilén (ref 19) built polydisperse spectra by additive mixing "not considering… plasmon coupling" and attributes the sim-vs-experiment gap to exactly that; and Bilén further finds that **polydispersity barely imprints on a monomer-only spectrum** (a 1–8 nm spread produces only minor broadening). That last point is close to a direct measurement of the central hypothesis here: if realistic polydispersity barely broadens the monomer spectrum, the large excess width observed at this spread cannot be polydispersity — it must be coupling / speciation.
...
**(e) MEASURED — γ(T) IS IN THE MODEL, AND THE VERDICTS ARE NOW RE-MEASURED
under the evaporation-aware normalization (CLAUDE.md #14 fixed) + per-T blank
subtraction. Outcome: the speciation-collapse conclusion SURVIVES the corrected
normalization AND both ε(T) treatments (ΔH₂ ≈ 0, agg ~T-flat); the pedestal is
REAL (survives both fixes) but HALF its fitted T-growth was the Kell artifact
(+31% → +9–16%), and its SHAPE is ε(T)-model-dependent (n_sca 0.00 under the
Drude-retune vs 0.85 under the full Reddy DCP, which fits better — unsettled);
the isosbestic crossing EXISTS but DRIFTS (~544→568 nm heating; not stationary
⇒ not two-state — the Kell-era "no crossing" was itself an artifact).**
ε(λ,T) is
